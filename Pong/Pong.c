...
int mainMenu() {
	...
	selected = menuSelector(x, y, yStart);	//Im Menü navigieren
...
int setschwierigkeitsgrad(str_player* player, int grosse) {
	...
	//Spieler länge muss durch 5 teilbar sein, da wir ihn in der Kollision in 5 Bereiche aufteilen
	switch (difficulty) {
	case 0:
		player[0].length = (CONSOLE_WIDTH / 5);	//Schwiereigkeitsgrad nicht von Spielgröße abhängig
	...
	case 2:
		if (grosse==1) {
			player[0].length = (CONSOLE_WIDTH / 15);	//Hier wäre sonst Spieler Länge 150/20 = 7.5!!!!!!!!!
